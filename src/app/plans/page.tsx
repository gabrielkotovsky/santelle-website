...
const trackGTMEvent = (eventName: string, eventData: Record<string, any>) => {
  if (typeof window !== "undefined") {
    const dataLayer = (window as any).dataLayer as
...
const commonFeatures = ["Full access to app", "30% off on extra kits"];
...
function PlansContent() {
  ...
  // Debug logging
  console.log("Plans page loaded with:", {
    recommendedParam,
  ...
  const [pendingPlan, setPendingPlan] = useState<(typeof allPlans)[0] | null>(
    null
  );
  ...
  const confirmPreOrder = async () => {
    ...
    trackGTMEvent("Disclaimer_Confirmed", {
      ...
      billing_type:
        pendingBillingPeriod === "one_time" ? "one_time" : "per_cycle",
  ...
  const recommendedPlan =
    recommendedPlanIndex !== null
      ? sortedPlans.find(
          (plan) => plan.originalIndex === recommendedPlanIndex
        ) || null
      : null;
  ...
  const displayedPlans =
    showAllPlans || !recommendedPlan
      ? sortedPlans
      : [
          recommendedPlan,
          ...(oneOffPlan &&
          oneOffPlan.originalIndex !== recommendedPlan.originalIndex
            ? [oneOffPlan]
            : []),
        ];
          ...
          <div className="bg-white/40 backdrop-blur-md rounded-3xl p-8 md:p-12 border border-white/50">
            <div className="text-center mb-8">
              ...
              <p className="text-lg text-[#721422]/80 mb-6">
                Welcome to Santelle! You&apos;ll receive a confirmation email
                shortly.
              </p>
              {detailsLoading && (
          ...
          <div className="bg-white/40 backdrop-blur-md rounded-3xl p-8 md:p-12 border border-white/50">
            <div className="text-center">
              <p className="text-xl text-[#721422] mb-6">
                Order canceled -- continue to shop around and checkout when
                you&apos;re ready.
              </p>
            </div>
              ...
              {isPendingOneOff ? (
                <>
                  <p className="font-semibold text-lg">
                    IMPORTANT: Start exploring the app as soon as you order your
                    kit — full access is unlocked instantly for 30 days.
                  </p>
                  <p>
                    You won’t need a subscription to use Santelle during this
                    time. Your one-off purchase includes complete access to
                    insights, tracking, and personalized recommendations for an
                    entire month after your test results.
                  </p>
                  <p>
                    After 30 days, access will automatically close unless you
                    choose to subscribe — no hidden fees, no automatic renewals,
                    no strings attached.
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p>
                    You&apos;ll get full access to the Santelle app immediately.
                    We won&apos;t charge you anything until your first kit is
                    ready to ship.
                  </p>
                  <p>
                    We&apos;ll email you at least 48 hours before your first
                    payment, so you have plenty of time to decide.
                  </p>
                  <p>
                    You can cancel your subscription anytime before your kit
                    ships — completely free, no questions asked.
                  </p>
                </>
              )}
                  ...
                  <div className="mt-auto">
                    <div className="text-center mb-6">
                      <div className="text-2xl sm:text-3xl font-bold text-[#721422]">
                        ...
                        {plan.cyclePeriod && (
                          <>
                            <span className="hidden sm:inline text-base sm:text-lg font-normal">
                              {" "}
                              / {plan.cyclePeriod}
                            </span>
                            <span className="block text-base sm:text-lg font-normal sm:hidden">
                              {isOneOff
                                ? plan.cyclePeriod
                                : `/ ${plan.cyclePeriod}`}
                            </span>
